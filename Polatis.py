...
def polatis_oxc(port, oxc):
    ...
    Example: polatis_oxc(2,194) will connect Ingress port 2 with Egress port 194
    """
    logger.warn('SNMP write to polatis to cross-connect {},{}'.format(port,oxc))
    ...
    g = setCmd(SnmpEngine(),
               ...
               ObjectType(
                   # First Port
                   ObjectIdentity('1.3.6.1.4.1.26592.2.2.2.1.2.1.2.{}'.format(port)),
                   # Second Port
                   Unsigned32(oxc)
               )
              )
